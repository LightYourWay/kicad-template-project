...
```
It asks for the repository name, the PBS number and a board title, renames `template.kicad_*` accordingly, fills the title block (`#122-000 Board Title`, date), runs `setup-libs.py` (see below) and deletes itself. It then offers to fold everything into the single commit GitHub created from the template, reworded to `feat: :tada: initialise project from template` – review with `git show --stat` and `git push --force-with-lease`. Decline (or `--no-amend`) and it only stages, so you commit yourself. `python3 init.py --help` lists the flags that pre-answer the prompts.
