...
```
It asks for the repository name, the PBS number and a board title, renames `template.kicad_*` accordingly, fills the title block (`#122-000 Board Title`, date), runs `setup-libs.py` (see below) and deletes itself. Review with `git status` and commit. `python3 init.py --help` lists the flags that pre-answer the prompts.
